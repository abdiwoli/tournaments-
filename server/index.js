...
const app = express();
// Expo Web runs on a different origin from the API. Keep the API usable from
// the configured web client and local Expo/Vite development servers.
...
app.post('/api/entities/:entity', requireAuth, route(async (req, res) => { const entity = req.params.entity; if (!entities.has(entity)) return res.status(404).json({ error: 'Unknown entity' }); const record = { ...req.body }; if (entity === 'Tournament') record.created_by_id = req.user.id; else if (entity === 'TournamentPermission') { const tournament = await getTournament(record.tournament_id); if (!tournament) return res.status(404).json({ error: 'Tournament not found' }); const isSelfFollow = !record.user_id || record.user_id === req.user.id; if (isSelfFollow) { const existing = await membership(tournament.id, req.user.id); if (existing) return res.status(200).json(existing); record.user_id = req.user.id; record.user_email = req.user.email; record.user_name = req.user.full_name || ''; record.role = 'follower'; } else { if (!(await hasRole(tournament, req.user, ['admin']))) return res.status(403).json({ error: 'Only the owner can add staff.' }); record.role = normalizedMemberRole(record.role || 'follower'); if (!memberRoles.has(record.role)) return res.status(400).json({ error: 'Invalid membership role.' }); if (await membership(tournament.id, record.user_id)) return res.status(409).json({ error: 'This user already follows the tournament.' }); } record.tournament_owner_id = tournament.created_by_id; } else if (!(await canWrite(entity, record, record, req.user))) return res.status(403).json({ error: 'You do not have permission to modify this tournament.' }); const stamp = now(), recordId = id(); await pool.query('INSERT INTO records (entity,id,data,created_date,updated_date) VALUES ($1,$2,$3::jsonb,$4,$5)', [entity, recordId, JSON.stringify(record), stamp, stamp]); res.status(201).json({ id: recordId, ...record, created_date: stamp, updated_date: stamp }); }));
